...
class SerieCapitlesVideoLinksGrabber
  ...
  def initialize(download_data)
    raise "No serie especified, exiting job" unless download_data
    @serie = {:title => download_data[:serie], :url => download_data[:url]}
    @season, @capitle= [download_data[:season], download_data[:capitle]]
  ...
  def do_job
    begin
      init
      data = get_capitles_links
      puts "Job done: #{self}"
    rescue
      puts "Error on Job #{self}"
      #TODO: crear alerta aqui
      data = [data]
    ensure
      data.compact!
    end
  ...
  def get_capitles_links
    @links = []
    return get_single_capitle if @capitle
  ...
  def to_s
    "#{@serie[:title]}, episode: #{@season ? @season.to_s+','+@capitle.to_s : 'ALL'}"
  end
  
  private 

  def get_single_capitle
    puts "Fetching single capitle"
    capitle = get_capitle_links_for_season(@season).select { |capitle| capitle[:order] = @capitle }.first
    raise "Capitle links not found" and return nil if capitle.blank?
    puts "Selected single capitle #{@season}, #{@capitle} from season, fetching videos..."
    capitle_videos_links = get_capitle_video_links(capitle)
    capitle_data = {
      :serie => @serie[:title],
      :season => @season,
    ...
    #persist_season_data(@season, [capitle_data])
    [capitle_data]
  end
  ...
  def persist_season_data(season, season_capitles_and_video_links)
    puts "Persisting data for season #{season}: #{season_capitles_and_video_links.count} capitles"
    @@mutex.synchronize do
      CSV.open("sy-export-daily-#{Date.today.gsub(' ', '_')}.csv", "a", {:col_sep => ';', :force_quotes => true}) do |f|
